...
function SortableTodoRow({ todo, onToggle, onEdit, onDelete }: RowProps) {
  ...
  return (
    ...
    >
      ...
      <button
        {...attributes}
        {...listeners}
        className="shrink-0 w-5 h-5 flex items-center justify-center rounded text-[#3A3A3A] hover:text-[#636366] transition-colors cursor-grab active:cursor-grabbing touch-none"
        aria-label="순서 변경"
        tabIndex={-1}
      >
...
export function TodoList({ todos, isLoading, otherDatesCount, onToggle, onEdit, onDelete }: Props) {
  const [ordered, setOrdered] = useState<Todo[]>(todos)

  // todos prop이 바뀌면(필터 변경, 외부 업데이트) 순서 초기화
  useEffect(() => {
    setOrdered(todos)
  }, [todos])
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (from !== -1 && to !== -1) setOrdered(arrayMove(ordered, from, to))
  }
  ...
  if (ordered.length === 0) {
    return (
      <div className="py-16 flex flex-col items-center justify-center gap-2">
        <p className="text-[13px]" style={{ color: '#8E8E93' }}>이 날짜에 할 일이 없어요</p>
